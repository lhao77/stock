...
class Market():
    ...
    def MktAdjf(self, field='', secID='', ticker='', exDivDate='', beginDate='', endDate=''):
        """
            获取获取沪深A股和B股用来调整行情的前复权因子数据，包含除权除息日、除权除息事项具体数据、本次复权因子、累积复权因子以及因子调整的截止日期。该因子用来调整历史行情，不作为预测使用，于除权除息日进行计算调整。
        """
        code, result = self.client.getData(vs.MKTADJF%(field, secID, ticker, exDivDate, beginDate, endDate))
        return _ret_data(code, result)
